...
contract ENS721Mapper is Ownable {
    ...
    string public domainLabel = "nouns";
    //need nouns image baseURI
    ...
    constructor(){
        ...
        nft = IERC721Enumerable(0x9C8fF314C9Bc7F6e59A9d9225Fb22946427eDC03);
   ...
   function getTokenDomain(uint256 token_id) private view returns(string memory uri){
    ...
    function getNounsWithDomains(address addy) public view returns(uint256[] memory){
        uint256 balance = nft.balanceOf(addy);
        uint256[] memory ids = new uint256[](balance);
        uint256 count;
        for(uint256 i; i < balance; i++){
